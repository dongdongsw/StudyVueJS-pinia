<template>
    <div class="container">
        ...
        <div class="row text-center" style="margin-top: 20px;">
            <ul class="pagination">
                <li v-if="find_data.startPage > 1"><a class="a-link"
                        @click="foodFindData(column, find_data.startPage - 1, ss)">&laquo;</a></li>
                <li v-for="i in range(find_data.startPage, find_data.endPage)" :key="i"
                    :class="find_data.curpage == i ? 'active' : ''">
                    <a class="a-link" @click="foodFindData(column, i, ss)">{{ i }}</a>
                </li>
                <li v-if="find_data.endPage < find_data.totalpage"><a class="a-link"
                        @click="foodFindData(column, find_data.endPage + 1, ss)">&raquo;</a></li>
            </ul>
        </div>
    </div>
</template>
